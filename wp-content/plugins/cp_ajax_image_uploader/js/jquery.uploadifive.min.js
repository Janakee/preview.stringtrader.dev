		...
		init:function(c){
			return this.each(function(){
				...
				d.drop=function(m){
					...
					var j=l.files.length;d.queue.selected=j;
				...
				d.fileExistsInQueue=function(k){
					for(var j in d.inputs){
						...
						for(var l=0;l<limit;l++){
							...
							if(existingFile.name==k.name&&!existingFile.complete){return true;}
						}
					}
					return false;
				};
				...
				if(f.itemTemplate==false){
					d.queueItem=b('<div class="uploadifive-queue-item">                        <a class="close" href="#" onclick="javascript:delete_queue(this);">X</a>                        <div><span class="filename"></span><span class="fileinfo"></span></div>                        <div class="progress">                            <div class="progress-bar"></div>                        </div><input type="hidden" name="attach_id[]" value="" class="attach_id">                    </div>');
				}else{
					...
					if(!k.skip&&!k.complete&&!k.uploading){
						...
						if(typeof FormData==="function"||typeof FormData==="object"){
							...
							xhr.upload.addEventListener("progress",function(n){
								if(n.lengthComputable){d.progress(n,k);}
							},false);
							...
							j.onload=function(q){
								var t="-------------------------"+(new Date).getTime(),p="--",o="\r\n",s="";s+=p+t+o;s+='Content-Disposition: form-data; name="'+f.fileObjName+'"';
								if(k.name){s+='; filename="'+k.name+'"';}
								s+=o;
								s+="Content-Type: application/octet-stream"+o+o;s+=q.target.result+o;
								for(key in f.formData){
									s+=p+t+o;s+='Content-Disposition: form-data; name="'+key+'"'+o+o;
									s+=f.formData[key]+o;
								}
								s+=p+t+p+o;
				...
				d.error=function(l,j,k){
					...
					d.uploadComplete=function(l,j,k){
						if(b.inArray("onUploadComplete",f.overrideEvents)<0){
							...
							// JCH
							var attach_id = jQuery.parseJSON(j.xhr.responseText).attach_id;
							j.queueItem.attr("attachment",attach_id);
							j.queueItem.find(".attach_id").val(attach_id);
							// JCH
						}
					...
					if(window.File&&window.FileList&&window.Blob&&(window.FileReader||window.FormData)){
						...
						if(!f.queueID){
							f.queueID=f.id+"-queue";d.queueEl=b('<div id="'+f.queueID+'" class="uploadifive-queue" />');
						...
						if(f.dnd){
							...
							h.addEventListener("dragleave",function(j){
								d.queueEl.removeClass("drag-over");
								j.preventDefault();
					...
					if(c){e.uploadFile.call(h,c);}
						...
						if((e.uploads.count+e.uploads.current)<f.uploadLimit||f.uploadLimit==0){
							...
							b("#"+f.queueID).find(".uploadifive-queue-item").not(".error, .complete").each(function(){
								...
								if((e.uploads.current>=f.simUploadLimit&&f.simUploadLimit!==0)||(e.uploads.current>=f.uploadLimit&&f.uploadLimit!==0)||(e.uploads.count>=f.uploadLimit&&f.uploadLimit!==0)){return false;}
								if(f.checkScript){
									_file.checking=true;
